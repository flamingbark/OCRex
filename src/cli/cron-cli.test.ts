...
type CronAddParams = {
  ...
  payload?: { model?: string; thinking?: string; message?: string; kind?: string };
...
describe("cron cli", () => {
  ...
  it("bootstraps autonomy loop with safe isolated defaults", async () => {
    await runCronCommand(["cron", "autonomy"]);
        ...
        if (method === "cron.list") {
          return {
            jobs: [
              {
                id: "job-old-1",
                name: "openclaw-self-improvement-loop",
              },
            ],
          };
        }
  ...
  it("fails autonomy bootstrap when duplicate name exists and --no-replace is set", async () => {
    resetGatewayMock();
    callGatewayFromCli.mockImplementation(
      async (method: string, _opts: unknown, params?: unknown) => {
        if (method === "cron.status") {
    ...
    const program = buildProgram();
    await expect(
      program.parseAsync(["cron", "autonomy", "--no-replace"], { from: "user" }),
    ).rejects.toThrow("__exit__:1");
  });
