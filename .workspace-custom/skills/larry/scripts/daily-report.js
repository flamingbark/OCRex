...
async function getRevenueCatMetrics(startDate, endDate) {
  ...
  try {
    ...
    const overviewRes = await fetch(
      `${RC_URL}/projects/${config.revenuecat.projectId}/metrics/overview`,
      {
        headers,
      },
    );
    ...
    const txRes = await fetch(
      `${RC_URL}/projects/${config.revenuecat.projectId}/transactions?start_from=${startDate.toISOString()}&limit=100`,
      {
        headers,
      },
    );
...
(async () => {
  ...
  // ==========================================
  const postsData = await postizAPI(
    `/posts?startDate=${startDate.toISOString()}&endDate=${now.toISOString()}`,
  );
  let posts = (postsData.posts || []).filter(
    (p) =>
  ...
  for (const app of apps) {
    const appPosts = postResults.filter((p) => p.app === app);
    const avgViews =
      appPosts.length > 0 ? appPosts.reduce((s, p) => s + p.views, 0) / appPosts.length : 0;
  ...
  for (const app of apps) {
    ...
    if (winners.length > 0) {
      ...
      const patterns = {
        personConflict: winnerTexts.filter(
          (t) =>
            /\b(showed|told|said|asked)\b/.test(t) &&
            /\b(my |mum|mom|dad|landlord|boyfriend|girlfriend|friend|sister|brother|boss|nan)\b/.test(
              t,
            ),
        ).length,
        pov: winnerTexts.filter((t) => t.startsWith("pov")).length,
        question: winnerTexts.filter((t) => t.includes("?")).length,
        emotion: winnerTexts.filter((t) =>
          /\b(obsessed|can't believe|shocked|never thought|didn't expect)\b/.test(t),
        ).length,
      };
    ...
    if (hasRC) {
      const highViewLowConvert = appHooks.filter(
        (h) => h.views > 10000 && (h.conversions || 0) === 0,
      );
