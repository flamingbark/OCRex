...
if (validate && configPath) {
  ...
  if (
    ...
    config.imageGen.provider !== "local" &&
  ...
  if (!config.postiz?.integrationIds?.tiktok)
    required.push("postiz.integrationIds.tiktok — TikTok integration ID");
  ...
  if (config.app?.isMobileApp && !config.revenuecat?.enabled) {
    optional.push(
      "RevenueCat — mobile app detected but RC not connected (recommended for conversion tracking)",
    );
  }

  // App Store link
  if (!config.app?.appStoreUrl)
    optional.push("App Store / website URL — helpful for competitor research");
  ...
  console.log(
    `   Image Gen: ${config.imageGen?.provider || "(not set)"}${config.imageGen?.model ? ` (${config.imageGen.model})` : ""}`,
  );
  console.log(
    `   TikTok: ${config.postiz?.integrationIds?.tiktok ? "Connected" : "Not connected"}`,
  );

  const crossPost = Object.keys(config.postiz?.integrationIds || {}).filter(
    (k) => k !== "tiktok" && config.postiz.integrationIds[k],
  );
  ...
  console.log(
    "  node onboarding.js --validate --config config.json    Validate config completeness",
  );
}
