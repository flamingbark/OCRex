...
if (provider === "openai" && model && !model.includes("1.5")) {
  ...
  console.warn(
    `   STRONGLY RECOMMENDED: Switch to "gpt-image-1.5" in your config for photorealistic results.`,
  );
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt < retries) {
        ...
        console.log(
          `  ⚠️ ${isTimeout ? "Timeout" : "Error"}: ${e.message}. Retrying (${attempt + 1}/${retries})...`,
        );
...
(async () => {
  console.log(
    `🎬 Generating 6 slides for ${config.app?.name || "app"} using ${provider}/${model}\n`,
  );
  let success = 0;
  let skipped = 0;
  for (let i = 0; i < 6; i++) {
