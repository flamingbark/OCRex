...
if (!configPath) {
  console.error(
    "Usage: node check-analytics.js --config <config.json> [--days 3] [--connect] [--app name]",
  );
  process.exit(1);
}
...
(async () => {
  ...
  console.log(
    `📊 Checking analytics (last ${days} days, cutoff: posts before ${cutoffDate.toISOString().slice(11, 16)} UTC)\n`,
  );

  // 1. Get all posts in range
  const postsData = await api(
    "GET",
    `/posts?startDate=${startDate.toISOString()}&endDate=${now.toISOString()}`,
  );
  ...
  if (appFilter) {
    posts = posts.filter((p) =>
      p.integration?.name?.toLowerCase().includes(appFilter.toLowerCase()),
    );
  }
  ...
  if (tooNew.length > 0) {
    ...
    tooNew.forEach((p) =>
      console.log(`    ⏳ "${(p.content || "").substring(0, 50)}..." — wait for TikTok to index`),
    );
  }
  console.log("");
  ...
  if (connectableUnconnected.length > 0 && shouldConnect) {
    ...
    if (Array.isArray(tiktokVideos) && tiktokVideos.length > 0) {
      ...
      for (let i = 0; i < connectableUnconnected.length; i++) {
        ...
        if (!videoId) {
          console.log(
            `  ⚠️ No matching video ID for "${(post.content || "").substring(0, 50)}..."`,
          );
          continue;
        }
      ...
      console.log(
        `  ⚠️ No TikTok videos found in missing list. Videos may need more time to index.\n`,
      );
    }
  } else if (connectableUnconnected.length > 0 && !shouldConnect) {
    console.log(
      `  ℹ️ ${connectableUnconnected.length} posts need connecting. Run with --connect to auto-connect.\n`,
    );
  }
  ...
  let updatedPosts = (updatedData.posts || []).filter(
    (p) =>
      p.integration?.providerIdentifier === "tiktok" && p.releaseId && p.releaseId !== "missing",
  );
  if (appFilter) {
  ...
  for (const post of updatedPosts) {
    ...
    console.log(
      `  ${result.date} | ${viewStr} views | ${result.likes} likes | ${result.comments} comments | ${result.shares} shares`,
    );
